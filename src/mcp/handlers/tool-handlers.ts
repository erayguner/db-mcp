...
export class QueryBigQueryHandler extends BaseToolHandler {
  async execute(args: unknown): Promise<ToolResponse> {
    try {
      ...
      const { query, dryRun, maxResults, timeoutMs, useLegacySql, location, confirmCost } =
        validated;
      ...
      if (costCfg.enabled && !confirmCost) {
        try {
          ...
          const estimatedBytes =
            Number.parseInt(String(estimate.totalBytesProcessed ?? '0'), 10) || 0;
          if (estimatedBytes > costCfg.thresholdBytes) {
            const estimatedCostUSD =
              estimate.estimatedCostUSD ?? estimateQueryCostUsd(estimatedBytes, costCfg.usdPerTiB);
